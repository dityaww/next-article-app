...
type EditArticleSchema = z.infer<typeof schema>

export default function FormEditArticle({ id } : { id: string }) {
    ...
    const { data: getArticleById } = useQuery({
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async (items: EditArticleSchema) => {
            try {
                const { data, status } = await axiosInstance.put(`/articles/${id}`, items, { headers: {
    ...
    useEffect(() => {
        if (getArticleById) {
            form.reset({
                title: getArticleById?.title || "",
                categoryId: getArticleById?.categoryId || "memek",
                content: getArticleById?.content || "",
            });
        }
    }, [getArticleById, form])
  ...
  return (
    <>
        <header className="flex gap-2 p-5 items-center">
            ...
            <div className="font-medium text-base leading-6">Edit Articles</div>
        </header>
                                ...
                                <SelectContent className="max-h-80">
                                    {categories && categories?.data?.length > 0 &&
                                        categories?.data?.map((data: any, index: number) => (
                                            <SelectItem key={index} value={data?.id || "reff"}>{data?.name || "-"}</SelectItem>
                                        ))
                                    }
                                </SelectContent>
                            </Select>
