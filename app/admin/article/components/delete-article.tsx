...
export default function DeleteArticle({ article, onSuccess }: { 
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async (id: string) => {
            try {
                const { data, status } = await axiosInstance.delete(`/articles/${id}`, { headers: {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="w-[400px]">
                ...
                <DialogDescription>
                    Delete article "{article.title}"? This will remove it from master data permanently
                </DialogDescription>
                ...
                <DialogFooter>
                    ...
                    <Button type="submit" onClick={() => mutate(article.id)} className="bg-red-600 hover:bg-red-700 disabled:bg-gray-300 disabled:text-gray-800" disabled={isPending}>{isPending ? "Loading.." : "Delete"}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
